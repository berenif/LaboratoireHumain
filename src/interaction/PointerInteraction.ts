...
export class PointerInteraction implements InteractionController {
  ...
  private bodyInputAvailable = true;
  ...
  consumeCommand(): GrabCommand | null {
    if (!this.bodyInputAvailable) return null;
  ...
  /** Drop input immediately; unlocking never manufactures a new press. */
  setBodyInputAvailable(available: boolean): void {
    const changed = this.bodyInputAvailable !== available;
    this.bodyInputAvailable = available;
    if (!available) {
      const pointerId = this.activeGrab?.pointerId ?? this.pendingBegin?.pointerId;
  ...
  getStatus(): Readonly<InteractionStatus> {
    ...
    return Object.freeze({
      bodyInputAvailable: this.bodyInputAvailable,
  ...
  private readonly handlePointerDown = (event: PointerEvent): void => {
    // A down received during lockout is discarded, including its target. A
    // held pointer's later moves cannot acquire a grab after standing returns.
    if (!this.bodyInputAvailable || !this.element || !this.projection || !this.pick) {
      return;
    }
  ...
  private readonly handlePointerMove = (event: PointerEvent): void => {
    if (!this.bodyInputAvailable) return;
    const grab = this.activeGrab;

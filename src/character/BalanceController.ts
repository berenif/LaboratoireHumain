import { SEGMENT_BY_ID, SEGMENTS, TOTAL_MASS_KG } from "../core/humanoid";
...
import { horizontal, midpoint, type StepMotion } from "./pose";
...
export const BALANCE_LIMITS = Object.freeze({
  ...
  pullDampingNsPm: 12,
  maxPullForceN: 620,
  maxBalanceAccelerationMps2: 2.6,
  maxRootSpeedMps: 2.5,
  stepTriggerMarginM: 0.055,
  maxStepReachM: 0.36,
  maxStepTravelM: 0.43,
  stepDurationS: 0.34,
  stepCooldownS: 0.065,
...
export class BalanceController {
  private feet: Record<Foot, Vec3> = { leftFoot: { x: -0.13, y: 0.065, z: 0.075 }, rightFoot: { x: 0.13, y: 0.065, z: 0.075 } };
  ...
  reset(poses: ReadonlyMap<SegmentId, SegmentPose>, heading = 0): void {
    ...
    this.nominalHeight = poses.get("pelvis")?.position.y ?? 1.03;
    this.velocity = horizontal(poses.get("pelvis")?.linearVelocity ?? ZERO);
    this.comVelocity = massState(poses).velocity;
    this.step = null;
    this.stepCount = 0;
    this.cooldown = 0;
    this.instability = 0;
  ...
  update(input: BalanceInput): BalanceOutput {
    ...
    if (this.step) {
      ...
      if (this.step.elapsed >= this.step.duration) {
        ...
        this.feet[this.step.foot] = { x: solved.x, y: floorY + 0.065, z: solved.z };
        this.stanceAge[this.step.foot] = 0;
        this.step = null;
    ...
    if (!this.step && !grabbedFoot) {
      const airborne = FEET.find((foot) => {
        ...
        return pose.position.y - floorY > 0.105 && length(horizontal(sub(pose.position, this.feet[foot]))) > 0.04;
      });
    ...
    for (const foot of FEET) {
      ...
      for (const x of [-1, 1]) for (const z of [-1, 1]) {
        corners.push(worldPoint(pose.position, pose.rotation, { x: x * 0.095, y: -0.065, z: z * 0.155 }));
      }
    }
    ...
    const stepCenter = this.step ? midpoint(this.feet[this.step.foot === "leftFoot" ? "rightFoot" : "leftFoot"], this.step.to) : supportCenter;
    const desiredCom = add(horizontal(stepCenter), scale(forward, -0.065));
    const balanceAcceleration = clampLength(add(
      scale(sub(desiredCom, horizontal(mass.position)), 24),
      scale(horizontal(this.comVelocity), -6.8),
    ...
    if (!this.step && this.cooldown === 0 && supportingFeet.length > 0
      ...
      let foot = dot(direction, right) > 0.25 ? "rightFoot" as const : dot(direction, right) < -0.25 ? "leftFoot" as const : this.nextFoot;
    ...
    if (this.step) {
      ...
      for (const x of [-1, 1]) for (const z of [-1, 1]) {
        landingCorners.push(worldPoint(this.step.to, heading, { x: x * 0.095, y: -0.065, z: z * 0.155 }));
      }
    ...
    const immediate = noSupport || supportMargin < BALANCE_LIMITS.unrecoverableMarginM
    ...
    const desiredReaction = clampLength(add(scale(horizontal(force), 0.0008), scale(sub(capturePoint, desiredCom), 0.65)), 0.62);
    this.reaction = lerp(this.reaction, desiredReaction, 1 - Math.exp(-dt * 9));
    const kneeFlexion = clamp(0.12 + length(this.reaction) * 0.7 + (this.step ? 0.32 : 0), 0, 1);
  ...
  private beginStep(foot: Foot, from: Vec3, root: Vec3, forward: Vec3, right: Vec3, floorY: number, correction: Vec3): void {
    ...
    const reach = clampLength(add(scale(forward, 0.075), add(lateral, correction)), BALANCE_LIMITS.maxStepReachM);
    const requested = add(root, reach);
    const travel = clampLength(horizontal(sub(requested, from)), BALANCE_LIMITS.maxStepTravelM);
    const to = { ...add(from, travel), y: floorY + 0.065 };
    this.step = { foot, from: { ...from }, to, elapsed: 0, duration: BALANCE_LIMITS.stepDurationS };

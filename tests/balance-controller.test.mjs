...
const { quatFromAxisAngle, quatMultiply, rotate, sub, length } = await import("../src/character/math.ts");
...
function poseInput(heading = 0) {
  ...
  return { rootTranslation: { x: 0, y: 1.03, z: 0 }, reactionOffset: zero,
    simulationTime: 0, activeGrab: null, step: null, heading,
    supportFeet: Object.fromEntries(["leftFoot", "rightFoot"].map(foot => [foot, rotate(yaw, { ...rest.get(foot).position, y: 0.065 })])),
  };
}
...
test("solved ankles stay connected for unreachable grabs and steps at rotated headings", () => {
  for (const heading of [0, Math.PI / 2, -0.7, 2.2]) {
    ...
    for (const side of ["left", "right"]) {
      ...
      const stepping = composeUprightPose({ ...base, step: { foot, from: start, to: { x: 3, y: 0.065, z: -2 }, elapsed: 0.30, duration: 0.34 } }).poses;
